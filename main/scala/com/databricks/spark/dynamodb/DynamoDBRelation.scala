...
case class DynamoDBRelation(location: String)(@transient val sqlContext: SQLContext) extends TableScan with PrunedScan, PrunedFilteredScan {
  ...
  //    case s: StructType => s
  //    case other =>
  //      sys.error(s"DynamoDB must contain Records to be read, type $other not supported")
  //  }
  //  fileReader.close()
  //  convertedSchema
  //}
...
//  }
  ...
  private def toSqlType(dynamoDBSchema: Schema): SchemaType = {
    ...
    avroSchema.getType match {
      ...
      case RECORD =>
        val fields = dynamoDBSchema.getFields.map { f =>
      ...
      case ARRAY =>
        val schemaType = toSqlType(dynamoDBSchema.getElementType)
        SchemaType(
          ArrayType(schemaType.dataType, containsNull = schemaType.nullable),
          nullable = false)

      case UNION => dynamoDBSchema.getTypes.toSeq match {
